...
[![GoDoc](https://godoc.org/github.com/bitgaming/packr/v2?status.svg)](https://godoc.org/github.com/bitgaming/packr/v2)
...
```text
$ go get -u github.com/bitgaming/packr/v2/...
```

## Binary Installation

```text
$ go get -u github.com/bitgaming/packr/v2/packr2
```
...
A: Packr works by searching your `.go` files for [`github.com/bitgaming/packr/v2#New`](https://godoc.org/github.com/bitgaming/packr/v2#New) or [`github.com/bitgaming/packr/v2#NewBox`](https://godoc.org/github.com/bitgaming/packr/v2#NewBox) calls. Because those files aren't "proper" Go files, Packr can't parse them to find the box declarations. To fix this you need to tell Packr to ignore those files when searching for boxes. A couple solutions to this problem are:
...
A: Packr attempts to "automagically" set it's resolution directory when using [`github.com/bitgaming/packr/v2#New`](https://godoc.org/github.com/bitgaming/packr/v2#New), however, for dynamic paths you need to set it manually:
...
A: Using [`github.com/bitgaming/packr/v2#Folder`](https://godoc.org/github.com/bitgaming/packr/v2#Folder) gives you back a `*packr.Box` that can be used as normal, but is excluded by the Packr tool when compiling.
...
A: Packr works by parsing `.go` files to find [`github.com/bitgaming/packr/v2#Box`](https://godoc.org/github.com/bitgaming/packr/v2#Box) and [`github.com/bitgaming/packr/v2#NewBox`](https://godoc.org/github.com/bitgaming/packr/v2#NewBox) declarations. If there aren't any `.go` in the folder that `packr2` is run in it can not find those declarations. To fix this problem run the `packr2` command in the directory containing your `.go` files.
